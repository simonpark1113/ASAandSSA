...
class UNetDecoder(nn.Module):
    def __init__(self,
        ...
        """
        print("decoder for unetDecoderSynthseg - Attention")
        super().__init__()
        ...
        for s in range(1, n_stages_encoder):
            ...
            synth_attn_layers.append(synthsegAttentionModule(input_features_skip*2, input_features_skip//2))
    ...
    def forward(self, skips, skips_label):
        """
        ...
        for s in range(len(self.stages)):
            ...
            skips_i = self.synth_attn_layers[s](skips[-(s+2)], skips_label[-(s+2)])
            x = torch.cat((x, skips_i), 1)
            x = self.stages[s](x)
