...
class AttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        self.ds_reduce_channels = Convolution(
            spatial_dims=spatial_dims,
            in_channels=out_channels,
            out_channels=2,
            kernel_size=1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if(self.is_bottom):
            lower_output = self.submodule(x)
        else:
            lower_output, ds_conv = self.submodule(x)
        lower_predict = self.ds_reduce_channels(lower_output)
        
        fromlower = self.upconv(lower_output)
        att = self.attention(g=fromlower, x=x)
        att_m: torch.Tensor = self.merge(torch.cat((att, fromlower), dim=1))
        
        if(self.is_bottom):
            return att_m, [lower_predict]
        else:
            return att_m, ds_conv + [lower_predict]
...
class AttentionUnet(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = deep_supervision_switch()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_m, ds_list = self.model(x)
        
        # print(len(ds_list))
        # for i in range(len(ds_list)):
        #     print(ds_list[i].shape)
        # torch.Size([4, 1, 8, 8, 8])
        # torch.Size([4, 1, 16, 16, 16])
        # torch.Size([4, 1, 32, 32, 32])
        # torch.Size([4, 1, 64, 64, 64])
        x_m = self.reduce_channels(x_m)
        
        final_list = ds_list + [x_m]
...
def build_model_ds(config):
    model_config = config['model']
    spatial_dims = model_config['spatial_dims']
    in_channels = model_config['in_channels']
    out_channels = model_config['out_channels']
    channels = ast.literal_eval(model_config['channels'])
    strides = ast.literal_eval(model_config['strides'])
    dropout = model_config['dropout']
    
    model = AttentionUnet(spatial_dims=spatial_dims, \
            in_channels=in_channels,\
            out_channels=out_channels, \
            channels=channels, \
            strides=strides, \
