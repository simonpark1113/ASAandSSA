...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        results['metrics'][r]['recall'] = tp / (tp + fn) if tp + fn > 0 else np.nan
        results['metrics'][r]['label_size'] = tp+fn
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    metric_list = list(results[0]['metrics'][regions_or_labels[0]].keys()) # "Dice", "FN", ...
    means = {}
    means_under = {}
    means_over = {}
    pivot_value = 5300
    
    # print("my compute metrics entered")
    # print("regions_or_labels: ", regions_or_labels)
    under_count = 0
    over_count = 0
    total_count = 0
    for r in regions_or_labels: # Only r = 1
        means[r] = {}
        means_under[r] = {}
        means_over[r] = {}
        for m in metric_list:
            means[r][m] = np.nanmean([i['metrics'][r][m] for i in results])
            means_under[r][m] = np.nanmean([i['metrics'][r][m] for i in results if i['metrics'][r]['label_size'] <= pivot_value])
            means_over[r][m] = np.nanmean([i['metrics'][r][m] for i in results if i['metrics'][r]['label_size'] > pivot_value])
        under_count = np.sum([1 for i in results if i['metrics'][r]['label_size'] <= pivot_value])
        over_count = np.sum([1 for i in results if i['metrics'][r]['label_size'] > pivot_value])
        total_count = np.sum([1 for i in results])

        
    print("under_count: ", under_count)
    print("over_count: ", over_count)
    print("total_count: ", total_count)
    # foreground mean
    foreground_mean = {}
    foreground_mean_under = {}
    foreground_mean_over = {}
    for m in metric_list:
        values = []
        values_under = []
        values_over = []
        for k in means.keys():
            ...
            values_under.append(means_under[k][m])
            values_over.append(means_over[k][m])
        foreground_mean[m] = np.mean(values)
        foreground_mean_under[m] = np.mean(values_under)
        foreground_mean_over[m] = np.mean(values_over)


    [recursive_fix_for_json_export(i) for i in results]
    recursive_fix_for_json_export(means)
    recursive_fix_for_json_export(means_under)
    recursive_fix_for_json_export(means_over)
    recursive_fix_for_json_export(foreground_mean)
    recursive_fix_for_json_export(foreground_mean_under)
    recursive_fix_for_json_export(foreground_mean_over)
    # print("means: ", means)
    # print("means_under: ", means_under)
    # print("means_over: ", means_over)
    # print("foreground_mean: ", foreground_mean)
    # print("foreground_mean_under: ", foreground_mean_under)
    # print("foreground_mean_over: ", foreground_mean_over)
    
    # SPARK: Original three mean values
    result = {'metric_per_case': results, 'mean': means, 'foreground_mean': foreground_mean, 
            'means_under': means_under, 'means_over': means_over, 
            'foreground_mean_under': foreground_mean_under, 'foreground_mean_over': foreground_mean_over}
    if output_file is not None:
